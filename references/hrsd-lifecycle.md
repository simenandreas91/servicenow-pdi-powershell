...
- Submit-catalog HR tasks update the existing HR case through record producers instead of creating new cases. Map the parent case number into a hidden/read-only Single Line Text variable, resolve the HR case with `hrCase.get(key) || hrCase.get('number', key)`, update the fields, then abort the producer create action.
...
- For submit-catalog HR tasks that need to update the parent HR case, use a hidden/read-only Single Line Text variable for the case key and map `sn_hr_le_case.number` into it. Mapping `sys_id` into a Reference variable can be rejected by the `Validate mapped reference variable` Business Rule; the producer script can resolve the case with `hrCase.get(key) || hrCase.get('number', key)` and then call `current.setAbortAction(true)` after updating the existing case.
