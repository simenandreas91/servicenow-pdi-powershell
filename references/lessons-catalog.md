...
- For Date catalog variables (`item_option_new.type=9`), validate relative windows with a Catalog Client Script rather than `attributes`: the PDI's existing date variable scripts use `getDateFromFormat(newValue, g_user_date_format)`, and `catalog_script_client.cat_variable` stores the target variable as `IO:<item_option_new sys_id>`. An onChange script can clear invalid values and show a field message; pair it with onSubmit when bypass prevention matters.

## Scoped Record Producers And Metadata Automation

- A scoped Xplore insert can fail silently for a record producer or another Global catalog metadata parent while later child inserts still succeed. Never create variables, user-criteria links, or catalog client scripts until the parent insert returned a non-empty sys_id and a no-cache re-read confirms the intended scope, package, target table, and name. If the parent failed, remove only the exact orphaned task-created children before retrying.
- For narrow PDI automation of `sc_cat_item_producer`, `item_option_new`, `user_criteria`, `sc_cat_item_user_criteria_mtom`, and `catalog_script_client`, prefer Table API writes with explicit `sys_scope` and `sys_package`. Use the source application update set, then verify the item through the Service Catalog API and compare the returned variable names/count with the design; a successful metadata POST alone is not producer proof.
- Treat the Service Catalog item API's variable projection as runtime evidence, not a lossless metadata export. A valid standalone CheckBox (`item_option_new.type=7`) can appear there as an unnamed type `0`, matching known-good OOTB producers. Before repairing it, re-read the exact `item_option_new` row and compare the projection with an OOTB checkbox. Validate choices by querying `question_choice.question` with the resolved variable sys_ids; an unsupported dot-walk such as `question.cat_item` can be discarded and return unrelated choices.
- Validate every reference qualifier against the referenced table's live dictionary before saving it. Common-looking fields are not universal across releases or table families; for example, do not add `active=true` to a location reference unless `cmn_location.active` actually exists in that instance. An invalid qualifier can leave the portal reference empty or behave inconsistently across channels.
- Treat dictionary `unique` and `sys_index.unique_index` as metadata to verify, not behavioral proof. After creating an index through the supported table/index path, perform a safe duplicate-insert test and confirm the second insert is rejected. If immediate enforcement is required and the supported index is not active yet, add a small before insert/update Business Rule that excludes the current record on update, then retain the database index for long-term integrity and query performance.
- Scoped Xplore scripts that manipulate Global catalog metadata can create task-specific Restricted Caller Access/cross-scope privilege records and noisy customer updates. Prefer Table API or the owning scope for construction, inspect the update-set summary for `Cross scope privilege`, and remove only privileges demonstrably created by the task and unnecessary at runtime.
