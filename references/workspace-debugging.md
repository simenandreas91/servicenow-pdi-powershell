...
| Date-Time-Interval rejects a week/year value or format | UTS #35 token case mismatch or a component validator that rejects repeated valid tokens; current Australia Horizon behavior accepts `w-Y` but can reject UTS-valid `ww-YYYY` | exact component `type`, `format`, start/end and disabled-item values, installed `@servicenow/now-date-time` version, then the same inputs on a direct OOTB component without a custom wrapper | for `type=week-year`, first test lowercase `w` plus uppercase `Y` as `w-Y`; it accepts values such as `36-2026` and can accept zero-padded `01-2026`; change a custom wrapper only if direct OOTB works and the wrapper hard-codes, transforms, or passes a different format/value shape; if direct OOTB fails, treat it as a version/regression issue; test weeks 1, 52, and 53 across year boundaries |
...
- [Date-Time-Interval component guidance](https://horizon.servicenow.com/workspace/components/now-date-time-interval?release=australia)
- [Unicode UTS #35 date field symbols](https://unicode.org/reports/tr35/tr35-dates.html#Date_Field_Symbol_Table)
