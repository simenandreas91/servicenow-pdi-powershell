...
- For STRY0010375 Department work in Vår Energi sandbox, reuse installed SuccessFactors assets before creating custom staging: table `sn_successfactors_departments` and inactive transform map `SuccessFactors Department Staging Table` (`feb607c973724010062b2ea3c4f6a798`) already map `dept_id` to `cmn_department.id` with coalesce plus `name`, `description`, `parent_department`, and `dept_head`. A non-versioned Scripted REST API with `namespace=company` and `service_id=masterdata` exposes default path `/api/company/masterdata/department`, while ServiceNow also shows versioned operation URI `/api/company/v1/masterdata/department`.
