...
- Re-check sandbox authentication before starting Compendia follow-up work. On 2026-06-18 the same `other` profile still authenticated to DEV and PROD, but sandbox returned HTTP 401 for both Table API and Xplore. A direct `login.do` local-login attempt with a provided sandbox credential also returned "User name or password invalid"; do not assume sandbox still shares DEV/PROD credentials.
