...
- On the Australia PDI, generated ACL records were readable but `sys_security_acl_role` inserts and `sys_security_acl` script updates through the Table API failed with security constraints. After proving the exact generated ACL natural keys and confirming the role links were absent, a narrow Global Xplore operation could add only the intended ACL-role links and owner-delete script. Re-read every link and validate with `GlideRecordSecure` as a temporary non-admin persona; never respond by making the table public or removing the row script.
...
- A `sys_ws_operation` created through Table API can reference `sys_ws_definition` while leaving `web_service_version` blank. ServiceNow then computes an unversioned `operation_uri`, and the documented `/api/<scope>/v1/...` URL reports that it is not a resource. Resolve the exact active `sys_ws_version` and set `web_service_version`, then verify the computed URI and live response.
- When Default form layouts are created through Table API and force-captured with Global `GlideUpdateManager2`, `sys_ui_element` customer updates can inherit `application=global` even though their parent `sys_ui_section` belongs to the custom app. Reconcile only those exact `Section Element` update XML rows to the scoped application's sys_id, then rerun the mixed-scope summary. Do not accept a mixed update set merely because the runtime form renders.
- Idempotent metadata setup must treat a deployed HTML system-property value as create-only. Rerunning setup must not replace a real single-file React bundle with a placeholder; property content replacement belongs to a separate, hash-reporting deployment helper.
